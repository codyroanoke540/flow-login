...
-- Membership records: users can see their own record; active members can see
-- their organization roster. Only owners/admins can add/change/remove others.
DO $$
DECLARE p text;
BEGIN
...
-- Replace broad FOR ALL member policies with explicit active-member reads and
-- role-restricted writes. This prevents a read-only member from bypassing the
-- server functions by writing directly through the Supabase client.
DO $$
DECLARE
  t text;
  p text;
...
-- Decision records are readable by active members and writable only by roles
-- permitted to run/review decisions in the application.
DO $$
...
-- Decision configuration: active members can read; owners/admins manage it.
DO $$
DECLARE t text; p text;
BEGIN
...
-- Audit history stays append-only. Only owner/admin may read the full log;
-- operational roles may insert the events produced by server functions/RPCs.
DO $$
DECLARE p text;
BEGIN
...
-- Organization settings are readable by active members and writable only by
-- owners/admins. Replace the earlier broad FOR ALL policy explicitly.
DO $$
DECLARE p text;
BEGIN
...
-- Integrity checks. Use NOT VALID first so the migration can be applied even
-- if legacy pilot rows need cleanup; new/updated rows are enforced immediately.
DO $$ BEGIN
...
-- The application uses explicit terminal/attention statuses beyond the original
-- four-value MVP check.
...
-- Keep at most one open recommendation per work item before enforcing it.
WITH ranked AS (
...
-- A recommendation receives one final approval/rejection record.
WITH ranked AS (
...
-- Atomic availability replacement. A failed insert rolls back the delete.
...
-- Atomic decision execution after the TypeScript engine has revalidated the
-- selected candidate. State changes and approval history commit together.
...
-- One terminal outcome per work item. Keep the newest legacy row before
-- enforcing the rule used by the pilot UI.
WITH ranked AS (
...
-- Record the terminal result and close the work item in one transaction.
