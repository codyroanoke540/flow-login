...
import { approveRecommendation, listRecommendations, rejectRecommendation } from "@/lib/cadence.functions";
...
function OperationsCenter() {
  ...
  const { data: liveRecs = [] } = useQuery({ queryKey: ["recommendations"], queryFn: () => listFn() });
  const pendingRecs = (liveRecs as any[]).filter((r) => r.status === "pending");
  const approveMut = useMutation({
    mutationFn: (id: string) => approveFn({ data: { id } }),
    onSuccess: () => { toast.success("Approved — schedule updated"); qc.invalidateQueries({ queryKey: ["recommendations"] }); qc.invalidateQueries({ queryKey: ["work_items"] }); },
    onError: (e) => toast.error(e instanceof Error ? e.message : "Failed"),
  });
  const rejectMut = useMutation({
    mutationFn: (id: string) => rejectFn({ data: { id } }),
    onSuccess: () => { toast.message("Rejected"); qc.invalidateQueries({ queryKey: ["recommendations"] }); },
  });
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-10 px-6 py-8">
      ...
      {pendingRecs.length > 0 && (
        <section>
          <div className="mb-3">
            <h2 className="font-display text-lg font-semibold tracking-tight">Live recommendations</h2>
            <p className="text-sm text-muted-foreground">Awaiting your approval — generated by the Cadence decision engine.</p>
          </div>
